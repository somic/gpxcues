...
and gives me cues based on lat/lon coordinates.

Cues can be text or sounds.

Possible options for cue delivery:
- on-screen (visual)
- verbal text played through TTS (audio)
- sounds such as beeps, chimes, clicks, drumsticks, etc (audio)


# Main Loop

1. I select a GPX file from local storage.
2. I select cue delivery options (through checkboxes) for this ride or walk.
3. I tap START.
4. App starts tracking my location along the route from the GPX file.
5. When current location is at or near a GPX trkpt with a cue, app delivers a cue.
6. At the end of the ride, I tap STOP.


# Examples of GPX file (fragments)

Use standard GPX extensions mechanism.

## Say text message through TTS
...
* Each cue can be delivered at most once
  - if I missed a cue because I was off route - drop it
...
* App will be getting GPS location at certain intervals (for example, every 10s)
...
* I could be passing through the same lat/lon (but different trkpt!) several times during a ride
  ...
  - when I intercept the route after being off-track, assume it's the closest trkpt to my last known on-track position


## Following the route

* On-track notifications
  - deliver a built-in "on-track" cue when I am on track and last cue was a while ago so that I know that the app
    didn't crash and that I am on track
  - also deliver this cue when I get on track after being off-track
* Off-track notifications
  - deliver a built-in "off-track" cue when I am off-track (both initially and during deviations)
...
* Show the cue at a very high screen brightness
* Turn off screen after configured time, do not waste the phone battery


# Tech Stack

* Android
* Development platform - Linux, Android tooling available in PATH
* No internet connection during rides (app should work without internet)
...
* App must reliably run in the background (ACTION_REQUEST_IGNORE_BATTERY_OPTIMIZATIONS)
  - when on-screen notifications are off, screen will be off and phone will be in a pocket
* GPS could be noisy or (rarely) unavailable
...
* distance since last cue for on-track notification (heartbeat)
* how frequently to send off-track notifications
* GPS fix interval - get GPS location once per this many seconds
* on-screen notification time - how long screen should stay on with a given cue
